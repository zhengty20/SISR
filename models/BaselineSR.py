...
class Block(nn.Module):
    def __init__(self, fea_dim, bias = True):
        ...
        self.filter1 = nn.Conv2d(fea_dim, fea_dim, kernel_size=3, padding=1, bias=self.bias)
        self.filter2 = nn.Conv2d(fea_dim, fea_dim, kernel_size=3, padding=1, bias=self.bias)
...
class BaselineSR(nn.Module):
    def __init__(self, scale, in_dim, fea_dim, num_blocks=5, bias=True):
        super(BaselineSR, self).__init__()
...
if __name__ == '__main__':
    ...
    model = BaselineSR(2, 3, 32, 5, bias=False)
