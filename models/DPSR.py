...
class DPSR(nn.Module):
    ...
    def forward(
        self, x, width_mult=1.0, subnet_expand_block=None, active_num_blocks=None
    ):
        ...
        active_num_blocks = int(active_num_blocks)
        if not 1 <= active_num_blocks <= self.num_blocks:
            raise ValueError(
                f"active_num_blocks must be in [1, {self.num_blocks}], "
                f"got {active_num_blocks}"
            )

        if subnet_expand_block is None:
            subnet_expand_block = self.subnet_expand_block
        subnet_expand_block = int(subnet_expand_block)
        if width_mult != 1.0 and not 1 <= subnet_expand_block <= active_num_blocks:
            raise ValueError(
                f"subnet_expand_block must be in [1, {active_num_blocks}] for "
                f"active_num_blocks={active_num_blocks}, got {subnet_expand_block}"
            )

        if width_mult == 1.0:
            y = self.head(x, width_mult=1.0)
            for block in self.body[:active_num_blocks]:
                y = block(y, output_channels=self.fea_dim)
        else:
            y = self.head(x, width_mult=0.5)
            for block_index, block in enumerate(
                self.body[:active_num_blocks], start=1
            ):
                output_channels = (
                    self.half_fea_dim
